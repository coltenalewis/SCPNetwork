  ...
  return (
    <section className="mx-auto max-w-6xl px-4 pt-16 text-white md:pt-24">
      <motion.p
        ...
        className="glitch-text text-xs uppercase tracking-[0.25em] text-cyan-100/80"
      >
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
      >
        <h1 className="mt-4 bg-gradient-to-r from-cyan-100 via-white to-orange-200 bg-clip-text text-5xl font-bold text-transparent md:text-7xl">Colten Lewis</h1>
      </motion.div>
    ...
    <section id="work-history" className={`reveal-on-scroll mx-auto mt-16 max-w-6xl px-4 py-8 ${panel}`}>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[2fr_1fr]">
        ...
        <div className="rounded-2xl border border-white/20 bg-gradient-to-br from-slate-900/80 via-[#101c34]/80 to-[#2f1a16]/80 p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-cyan-100/70">Work History Reference</p>
          <div className="code-window mt-4 rounded-xl border border-white/15 p-4 text-xs text-cyan-100/85">
            <p>{'>'} producer_mode --enabled</p>
            <p>{'>'} systems_pipeline --stable</p>
            <p>{'>'} art_ui_support --chesapeake_titles</p>
            <p>{'>'} live_ops_feedback_loop --active</p>
          </div>
          <p className="mt-4 text-sm text-slate-300">Dark recruiter panel aesthetic retained without shipping binary image assets.</p>
        </div>
      </div>
    </section>
  );
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => (
          <motion.article key={project.slug} whileHover={{ y: -4, scale: 1.01 }} className={`${panel} reveal-on-scroll p-4`}>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {project.tags.map((t) => (
                <span key={t} className="text-xs text-orange-200">
                  #{t}
                </span>
              ))}
            </div>
  ...
  return (
    <section className="mx-auto mt-14 max-w-6xl px-4">
      <article className={`${panel} reveal-on-scroll p-6`}>
        ...
        {aiIntegrationContent.intro.map((paragraph) => (
        ...
        <ul className="mt-3 space-y-3 text-slate-200">
          {aiIntegrationContent.applications.map((app) => (
            <li key={app.title}>
    ...
    <section className="mx-auto mt-14 max-w-6xl px-4 pb-12">
      <article className={`${panel} reveal-on-scroll p-6`}>
        ...
        <p className="mt-3 max-w-3xl text-slate-200">
          I produce and program Roblox experiences with a systems-first mindset: define clear goals, ship clean architecture, and run dependable
          live operations. I specialize in bridging creative direction, technical execution, and production leadership.
        </p>
      </article>
      <footer className="mt-8 border-t border-white/20 pt-4 text-sm text-cyan-100">
        <div className="flex flex-wrap gap-3">
          ...
          <a href={profileLinks.creatorHub} target="_blank">
            Creator Hub
          </a>
          <a href={profileLinks.chesapeake} target="_blank">
            Chesapeake Studios
          </a>
